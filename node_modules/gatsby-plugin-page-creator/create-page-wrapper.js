...
function createPage(filePath, pagesDirectory, actions, ignore, graphql) {
  ...
  if (pathIsCollectionBuilder(absolutePath)) {
    ...
    if (!process.env.GATSBY_EXPERIMENTAL_ROUTING_APIS) {
      throw new Error("PageCreator: Found a collection route, but the proper env was not set to enable this experimental feature. Please run again with `GATSBY_EXPERIMENTAL_ROUTING_APIS=1` to enable.");
    }

    (0, _createPagesFromCollectionBuilder.createPagesFromCollectionBuilder)(filePath, absolutePath, actions, graphql);
    return;
